...
from my_loss_computor import compute_fastrcnn_loss


#1.整合面积乘法因子   TODO 2.对于osr整合后，改造损失函数计算过滤条件
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets,box_regression_pix):
    # type: (Tensor, Tensor, List[Tensor], List[Tensor]) -> Tuple[Tensor, Tensor]
    #该损失函数中，采样过滤了class_logits，labels，作为最后计算损失函数的部分，因此使用该部分的值，作为osr的训练输入为最佳
    classification_loss, box_loss = compute_fastrcnn_loss(class_logits, box_regression, labels, regression_targets,box_regression_pix)
...
class RoIHeads(nn.Module):
    ...
    def forward(self,
        ...
        #TODO 此处，是否可以暂存特征av和label


        result: List[Dict[str, torch.Tensor]] = []
        losses = {}
        if self.training:
            # 获取预测框对应的真实坐标，形式为xyxy
            box_regression_pix = proposals
            box_regression_pix = torch.cat(box_regression_pix, dim=0)

            #选取检测对象的大小来做平衡参数
            # box_regression-shape n，4
            assert labels is not None and regression_targets is not None
            loss_classifier, loss_box_reg = fastrcnn_loss(
                class_logits, box_regression, labels, regression_targets,box_regression_pix)
            losses = {
            ...
            #TODO train的时候，将class_logits，labels 也返回
        else:
